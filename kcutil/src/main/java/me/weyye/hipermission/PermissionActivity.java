...
import static com.kenning.kcutil.utils.other.FunExtentionKt.getStringResource;
...
public class PermissionActivity extends AppCompatActivity {
    ...
    private String getPermissionTitle() {
        return TextUtils.isEmpty(mTitle) ? getStringResource(R.string.开启XX, mAppName) : mTitle;
    }

    private void showPermissionDialog() {

        String title = getPermissionTitle();
        String msg = TextUtils.isEmpty(mMsg) ? getStringResource(R.string.为了您能正常使用XX需要以下权限, mAppName) : mMsg;
...
//                        onDeny(permissions[i], i);
                ...
                if (mCheckPermissions.size() > 0) {
                    onDeny(mCheckPermissions.get(mRePermissionIndex).Permission, 0);
                    finish();
//                    //用户拒绝了某个或多个权限，重新申请
